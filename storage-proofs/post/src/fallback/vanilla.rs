...
impl<'a, Tree: 'a + MerkleTreeTrait> ProofScheme<'a> for FallbackPoSt<'a, Tree> {
    ...
    fn prove_all_partitions_with_callback<'b>(
        ...
        if tree_cb.is_some() {
            let mut proof_buf = vec![0u8; pub_inputs.sectors.len() * pub_params.challenge_count * 8192]; // TODO: enough?
            let proof_len = (tree_cb.unwrap())(num_sectors_per_chunk, proof_buf.as_mut_ptr() as *mut libc::c_char, proof_buf.len());
